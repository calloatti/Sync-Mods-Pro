			...
			var masterStatuses = new Dictionary<string, ModStatus>
	  {
		...
		{ "Calloatti.SyncModsPro.Status.New", ModStatus.New },
			...
			foreach (var kvp in masterStatuses)
			{
				...
				statusToggle.style.marginLeft = 8;
				statusToggle.style.fontSize = 12;
			...
			Toggle toggleEnabled = new Toggle();
			toggleEnabled.AddToClassList("game-toggle");
			toggleEnabled.style.scale = new StyleScale(new Vector2(0.9f, 0.9f));
			toggleEnabled.style.marginLeft = 16;
			toggleEnabled.style.fontSize = 12;
			toggleEnabled.text = $"Enabled ({staticEnabledCount})";
			toggleEnabled.SetValueWithoutNotify(false);
			toggleEnabled.RegisterValueChangedCallback(evt =>
			{
				_filterBySavedEnabled = evt.newValue;
				ApplyFilters();
			});

			filtersContainer.Add(toggleEnabled);
